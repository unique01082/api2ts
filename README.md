## 介绍
[![GitHub Repo stars](https://img.shields.io/github/stars/unique01082/api2ts?style=social)](https://github.com/unique01082/api2ts)
[![npm (scoped)](https://img.shields.io/npm/v/@baolq/api2ts)](https://www.npmjs.com/package/@baolq/api2ts)
![GitHub tag (latest SemVer pre-release)](https://img.shields.io/github/v/tag/unique01082/api2ts?include_prereleases)
...
在 ```package.json``` 的 ```script``` 中添加 api: ```"api2ts": "api2ts",```
...
| apiPrefix  | 否 | api 的前缀 | string \| function | - |
| dedupeApiPrefix | 否 | 静态前缀已存在时避免重复添加 | boolean | true |
...
| mockConfig | 否 | mock 输出配置；`msw: true` 时生成对象形式 handler | `{ msw?: boolean }` | `{}` |
| enumStyle  | 否 | 枚举样式 | string-literal \| enum | string-literal |
| declareType | 否 | 对 object schema 优先生成 interface；复杂 alias 仍使用 type | type \| interface | type |
| formDataJsonBlob | 否 | multipart 中将 object 字段编码为 `application/json` Blob | boolean | false |
...
Các tùy chọn mới giữ nguyên hành vi mặc định cũ:

```ts
export default {
  schemaPath: 'http://petstore.swagger.io/v2/swagger.json',
  serversPath: './servers',
  // Opt in only when the receiving API expects JSON multipart parts.
  formDataJsonBlob: true,
  // Opt in to object-valued mock handlers and .ts filenames.
  mockConfig: { msw: true },
  // Set false only when a repeated literal prefix is intentional.
  dedupeApiPrefix: false,
  // Object-shaped declarations become interfaces when valid.
